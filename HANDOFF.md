...
### 커밋
`c3daa76` — docs(pro): "Living Speaker" 아키텍처 브리프 기준 gap analysis (진단만, 코드 변경 없음)
...
실제 값 검증이 없었음. mobile과 동일 구조로 구현(상세 배경/설계 근거는 mobile
HANDOFF.md 참고 — 여기선 Pro 쪽 차이점 위주로 기록).

### 구현 중 발견한 치명적 문제
**`dsp_controller.dart:175`가 `profile.adapterFactory`를 거치지 않고
`Adau1701Adapter`를 직접 생성하고 있었다.** 이 함수(`sendToDsp()`)가 Pro의 실제
유일한 하드웨어 전송 경로(슬라이더 편집/AI 적용/프리셋 로드 3개 진입점이 전부
`state`만 바꾸고, "SEND" 버튼을 눌러야 최종적으로 이 함수 하나가 실행되는 구조 —
`dsp_screen.dart:47`)라서, 이걸 놓쳤다면 새로 만든 SafetyValidator가 완전히
무의미해질 뻔했다. `profile.adapterFactory(rawWrite)` 사용으로 수정.

### 수정 내용
- `lib/core/dsp_safety.dart`(신규): `DspSafety` — mobile과 동일 임계값/로직
  (트위터 채널 게인 +6dB 상한, 트위터 HPF 최소 1500Hz, 2kHz 이상 부스트/200Hz 이하
  부스트 밴드 검증, biquad 역산 검증). 파일을 그대로 복제 구현(공유 패키지 없음 —
  ACM 계층의 기존 중복 패턴과 동일)
- `lib/core/dsp_safety_notice.dart`(신규): 전역 `GlobalKey<ScaffoldMessengerState>`로
  스낵바. `main.dart`의 `MaterialApp(scaffoldMessengerKey: ...)`에 연결(그동안 `const
  MaterialApp`이었는데 GlobalKey가 컴파일타임 상수가 아니라 `const` 제거)
- `lib/core/dsp/validating_dsp_adapter.dart`(신규): `ValidatingDspAdapter implements
  DspAdapter` — mobile과 동일 구조
- `system_profile.dart`: 3개 프로파일 `adapterFactory`가 항상 `ValidatingDspAdapter`로
  감싼 인스턴스만 반환하도록 수정, 채널 리스트를 top-level const로 추출
- `dsp_controller.dart`: 위 치명적 문제 수정. `Adau1701Adapter` 직접 import도 제거
  (더 이상 필요 없음)
- 부수 발견: `SafetyProfile.clampBassBoost`(dsp_engine.dart:317)가 **정의만 돼 있고
  어디서도 호출되지 않는 죽은 코드**였음 — 이번에 추가한 `DspSafety`가 Pro에서는
  사실상 유일하게 실제로 동작하는 저역 보호임

### 우회 불가능한 구조인지 스스로 검증한 근거
1. `grep -rn "Adau1701Adapter(\|Adau1466Adapter("` 결과, `system_profile.dart`의
   `adapterFactory` 클로저 안에서만 두 어댑터를 직접 생성함 — 수정 후 다른 파일 0건
2. 유일한 예외였던 `dsp_controller.dart:175`를 수정 완료
3. `peq_controller.dart`(별도 PEQ 화면 컨트롤러)는 grep 결과 하드웨어 write 코드가
   전혀 없음(로컬 상태만 관리) — 확인 완료, 별도 우회 경로 아님
4. `measurement_mic_screen.dart`(트위터 경고 문구가 있는 화면)도 grep 결과 자체
   write 경로 없음 — 최종적으로 `dsp_controller.dart:sendToDsp()`에 위임하는 구조라
   중복 확인 불필요
5. 위반 시 `DspSafetyNotice.show()`가 무조건 호출되므로 조용히 값만 바뀌는 경우는 없음

### Do not 준수
- Factory/User 분리(C), AKG 프로필 모델(H)은 손대지 않음
- 기존 UI 경고 문구(`measurement_mic_screen.dart:388-389`)는 그대로 둠

### 확인
`flutter analyze` — 0 issues (기존 무관 info 1건 `connect_controller.dart`
unnecessary_import 제외). **실기기/UI 실측 테스트는 못 함** — 코드 리뷰 수준으로만 검증

### 알려진 한계
mobile HANDOFF.md와 동일 — 임계값이 드라이버 스펙 미연동 보수적 기본값, `validateBiquad`
위반 처리가 passthrough 전체 무효화, mobile/Pro 코드 100% 복제.

### 다음 세션
C(Factory/User 레이어 분리) 진행 여부는 mobile 쪽에서 논의 후 이어서 진행.

### 커밋
`2d23537` — feat(pro): Safety Validation Layer 구현 — AOS 항목 D, 3개 진입점 강제 적용
...
### 배경
직전 gap analysis: Pro는 모든 프리셋이 `dsp_presets`(이름 리스트) +
`dsp_preset_$name`이라는 하나의 flat 네임스페이스에 저장돼서, "Factory"란 이름의
유저 프리셋이 실제 Factory를 그냥 덮어쓸 수 있었음(애초에 "진짜 Factory"라는 개념
자체가 코드에 없었음 — `resetAll()`이 `DspState.initial()`을 직접 호출하는 것과
유저가 이름으로 저장하는 프리셋이 서로 무관하게 존재).

### 수정 내용

**신규: `lib/core/factory_preset.dart`**
- `FactoryPreset` 불변 클래스(setter 없음) — `id`/`name`/`description`/
  `build(int maxPeqBands) → DspState`
- `kFactoryPresetFlat` — "보정 없는 평탄 기준값". **`DspState.initial()`을 그대로
  위임**해서 새로 값을 만들지 않음 — `resetAll()`과 Factory 로드가 항상 같은
  소스를 가리키게 하기 위함(따로 유지되며 드리프트할 여지 자체를 없앰)
- `isReservedPresetName(name)` — `kFactoryPresets`의 이름과 대소문자 무관하게
  비교. SharedPreferences 등 어떤 저장소에도 쓰지 않고 코드에 직접 내장

**`dsp_controller.dart` 전면 수정:**
- 생성자 초기 상태를 `DspState.initial(...)` 직접 호출 대신
  `kFactoryPresetFlat.build(...)`로 변경(위와 동일한 단일 소스 원칙)
- `resetAll()`도 동일하게 `kFactoryPresetFlat.build(...)` 사용 — **"롤백/Safe
  Preset 기능이 Factory 레이어를 가리키는지" 확인 요구사항을 이 방식으로 충족**:
  reset과 Factory 로드가 코드상 같은 함수를 호출하므로 구조적으로 항상 같은 값
- 신규 `loadFactoryPreset(FactoryPreset)` — SharedPreferences를 전혀 거치지 않고
  코드 내장 값을 바로 `state`에 반영(저장/삭제 대상이 아님)
- `savePreset(name)`의 반환 타입을 `Future<void>` → `Future<bool>`로 변경 —
  `isReservedPresetName(name)`이면 저장하지 않고 `false` 반환(저장 함수 자체가
  이름 충돌을 원천 차단, 호출부가 사용자에게 안내)
- 신규 `_migrateLegacyFactoryNamedPresets()` — 생성자에서 1회 실행. 분리 이전에
  "Factory"란 이름으로 저장돼 있던 유저 프리셋을 찾으면(대소문자 무관) **삭제하지
  않고 데이터를 보존한 채 "(사용자 저장본)"으로 이름만 변경**(이름 중복 시 번호
  자동 부여), `DspSafetyNotice`(직전 세션에 만든 전역 스낵바 알림, 이번에 프리셋
  마이그레이션 안내로도 재사용)로 사용자에게 통지. **선택한 마이그레이션 방침:
  "삭제 confirm"이 아니라 "자동 이름 변경 + 사후 통지"** — 데이터 손실 위험이
  없고 사용자 상호작용을 요구하지 않아 앱 시작 시점에 조용히(그러나 확인 가능하게)
  처리 가능하기 때문

**`dsp_screen.dart`:**
- 저장 다이얼로그: `savePreset`이 `false`를 반환하면 다이얼로그를 닫지 않고
  "'Factory'는 예약된 이름이라 사용할 수 없습니다" 안내 후 재입력 유도
- 불러오기 다이얼로그: 상단에 **"FACTORY (읽기전용)"** 섹션을 항상 표시(삭제
  버튼 없음), 그 아래 기존 "MY PRESETS" 섹션. 유저 프리셋이 0개여도 Factory가
  항상 있어서 더 이상 "저장된 프리셋이 없습니다"로 다이얼로그를 막을 필요 없음

### Safety Validation Layer(D) 확인 — 손대지 않고 통과 여부만 검증
Factory 로드는 `state`만 바꾸고(`loadFactoryPreset`), 실제 하드웨어 전송은 여전히
"SEND" 버튼 → `sendToDsp()`뿐이다. `sendToDsp()`는 직전 세션에 이미
`profile.adapterFactory(rawWrite)`(→`ValidatingDspAdapter`)를 쓰도록 고쳐놨으므로,
Factory를 불러온 뒤 SEND를 눌러도 동일하게 검증을 통과한다(코드 변경 없이 확인만 함).

### 부수 발견 (이번 스코프 아님, 손대지 않음)
`lib/features/peq/peq_controller.dart`가 완전히 별개의(사용되지 않는) PEQ
저장/불러오기 로직을 갖고 있음 — 어떤 화면에서도 참조되지 않는 죽은 코드
(`TunaiProShell._screens`에 없음, grep으로 참조 0건 확인)이고, 프리셋 저장 키에
`'preset_\$name'`(달러 기호가 이스케이프돼 있어 실제로는 문자열 보간이 안 되는
버그)도 있음. 이번 Factory/User 분리와 무관한 완전히 별개의 죽은 코드라 손대지
않음 — 필요하면 별도 세션에서 정리.

### 확인
`flutter analyze` — 0 issues (기존 무관 info 1건 제외). **실기기/UI 실측 테스트는
못 함** — 코드 리뷰 수준으로만 검증

### 다음 세션
H(AKG 프로필 모델) 진행 여부는 mobile 쪽에서 논의 후 이어서 진행.

### 커밋
`0257cd7` — feat(pro): Factory/User Layer 분리 — AOS 항목 C
...
### 배경
mobile과 동일 구조/원칙(상세 설계 근거는 mobile HANDOFF.md 참고). Pro는 기기 등록
인프라(mobile의 `device_service.dart` 같은 것)가 없어 `deviceId`는 구조상 필드만
있고 항상 `null` — User(로그인)는 `auth_controller.dart`가 있어 `userId`는 채워짐.

### 추가한 것
- `lib/core/akg/measurement_session.dart` — `MeasurementSession`(id, timestamp,
  deviceId(항상 null), userId, systemProfileId, channelCount) + Store(최근 200개)
- `lib/core/akg/user_preference_signal.dart` — mobile과 동일 구조

### 연결 지점
- `mic_measurement_controller.dart`: 생성자에 `Ref`가 없었어서 추가(`(ref) =>
  MicMeasurementController(ref)`) — 이 파일 유일한 생성 지점이라 다른 곳 영향 없음.
  `startMeasurement`/`startChannelMeasurement`가 각각 `MeasurementStatus.done`에
  도달할 때 `_recordSession()` 호출
- `dsp_controller.dart`: `resetAll`/`loadFactoryPreset`(→`rollback`),
  `savePreset`(→`save`), `loadPreset`(→`select`), `deletePreset`(→`delete`) 각각에
  `_recordPreferenceSignal()` 훅 추가 — 전부 fire-and-forget, 실패해도 프리셋
  흐름엔 영향 없음

### 확인
`flutter analyze` — 0 issues (기존 무관 info 1건 제외). 실기기 테스트는 못 함 —
코드 리뷰 수준으로만 검증.

### Gap Analysis 4개 항목 전체 마무리
B(있었음, 확인만) / C(완료, 2세션 전) / D(완료, 3세션 전) / H(완료, 이번 세션).
남은 후보 AIP(클라우드 동기화)는 의도적으로 스킵 — 상세는 mobile HANDOFF.md 참고.

### 다음 세션
다음 우선순위는 mobile 쪽에서 논의 후 이어서 진행(후보: 실기기 테스트 일괄 검증 /
AIP 착수 / mobile-Pro 코드 중복 해소).

### 커밋
`2ea6464` — feat(pro): AKG-ready Profile Model 추가 — gap analysis 항목 H (마지막)

---

## Pro 연결 디버깅 — ADAU1466 "직접 연결" 조사 및 TARGET 수동 선택 추가 (2026-07-05)

### 배경
Windows에서 "NO PORTS DETECTED", 장치관리자에 포트(COM & LPT) 카테고리 자체가 없음.
"1466은 USBi가 아니라 직접 연결 방식이었다"는 제보로 시작했으나, 조사 결과 **복구할
과거 구현 자체가 없었다** — git log 전수 확인 결과 `connect_controller.dart`는
initial commit부터 지금까지 UART(ICP5/CH34x)와 BLE(ICP5 자체 GATT fff0/fff2) 두
경로만 존재했고, ADAU1466은 이름 패턴(REFERENCE/TUNAI-REF/QCC5125/CS42448)으로
"탐지"만 되고 탐지되면 즉시 "지원 준비 중"으로 막아버리는 스텁이었다. 별도의
1466 전용 연결 로직이 삭제된 이력은 없음.

### 진짜 원인 — 플랫폼 한계 (버그 아님)
`pubspec.lock` 확인 결과 `flutter_blue_plus`는 android/darwin/linux 플랫폼 패키지만
있고 **Windows 패키지 자체가 없다.** 커밋 `fed6fff`("Windows에서 BLE 탭 숨김 및
크래시 방지")가 이미 이 이유로 Windows에서 BLE 탭을 의도적으로 숨겨놓은 상태 —
버그가 아니라 실제로 이 앱에서 Windows는 BLE를 쓸 수 없다. `_adau1466Names`
패턴(REFERENCE/QCC5125/CS42448)이 BLE advName 매칭용으로 쓰이는 걸 보면, ADAU1466
"파란보드"는 온보드 QCC5125 BLE로 직접 연결되는 구성일 가능성이 높다 — 그렇다면
**Windows에서는 이 앱으로 그 보드에 연결할 방법이 현재 없다**(COM 포트가 안 보이는
게 오히려 정상 — USB-시리얼 장치가 아예 없으니까). 이건 UI/컨트롤러 코드로 고칠 수
있는 문제가 아니라 BLE 스택 자체를 Windows 지원 플러그인으로 교체해야 하는 별도
스코프의 작업 — 이번 세션(Do not: 연결 UI/컨트롤러 레이어만 수정) 범위 밖이라
정직하게 화면에 안내만 추가함.

### 수정 내용
- **TARGET BOARD 드롭다운 신규 추가**: 기존엔 `_infoRow('TARGET', 'ADAU1701 via
  ICP5')`가 하드코딩된 텍스트였음(TARGET을 고를 방법 자체가 없었음 — "1466 옵션이
  사라졌다"는 제보의 실체). 이제 `kAllSystemProfiles`(TUNAI ONE/Isobarik/Reference)
  중 수동으로 선택 가능. 연결 중엔 비활성화, 자동탐지 성공 시 덮어써질 수 있음을
  안내 문구로 명시
- **ADAU1466 탐지 배너 문구 정정**: 기존 "지원 준비 중, ADAU1701만 가능"은 이제
  틀린 정보(이번 주 여러 세션에 걸쳐 Gain/Delay/PEQ가 실제로 구현·확정됨, XO만
  SafeLoad 미검증으로 잠겨 있음) — 정확한 현재 상태로 갱신하고, TARGET에서
  Isobarik/Reference 중 골라야 함을 안내(둘 다 이름 패턴이 겹쳐서 자동 구분 불가)
- **UART 포트 0개일 때 안내 추가**: 드라이버/케이블 확인 문구 + Windows에서는 BLE
  전용 보드를 지원 못 한다는 사실을 명시(자동탐지 실패를 "버그"로 오인하지 않게)
- STATUS 패널의 TARGET 행도 하드코딩 텍스트 대신 실제 선택된 `SystemProfile`을 표시

### Do not 준수
DSP 주소/Safety Validation/Factory 로직은 전혀 건드리지 않음 — `connect_screen.dart`
UI와 `systemProfileProvider` 갱신(기존에도 자동탐지가 하던 것과 동일한 종류의 상태
변경)만 수정.
...
### 다음 세션 (사용자 판단 필요)
1. 실제로 ADAU1466 "파란보드"가 어떤 인터페이스로 연결되는지 하드웨어 확인 필요
   (온보드 QCC5125 BLE뿐인지, 별도 UART 디버그 포트가 있는지)
2. BLE뿐이라면: Windows 지원 BLE 플러그인으로 교체(예: `flutter_reactive_ble`,
