...
//   AND project.connection == HardwareConnection.connected
...
//
// Tests:
//  1. ICP5 BLE connected + PASS_HANDSHAKE → sync project → canDeploy true
//  2. Disconnect / lose handshake → sync project → canDeploy false
...
bool canDeploy(ProProject? project) =>
    project != null &&
    project.dspTarget == 'ADAU1701' &&
...
void main() {
  group('DEPLOY button enablement', () {
    ...
    test(
      ...
      expect(canDeploy(proj), isFalse, reason: 'initially disconnected');
      ...
      proj = container
          ...
          .projects
          .firstWhere((p) => p.id == 'p1');
      expect(canDeploy(proj), isTrue, reason: 'connected after sync');
      ...
      proj = container
          ...
          .projects
          .firstWhere((p) => p.id == 'p1');
      expect(canDeploy(proj), isFalse, reason: 'disabled after disconnect');
    });
  });
}
